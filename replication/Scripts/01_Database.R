...
process_year <- function(target_year, data) {
  ...
  if (target_year == "1840") {
    ...
    base %>% mutate(census_pop=col("AOB001"), enslaved=0,
      black=col("APP002"),
      farmv_total=col("AOD001"),  # farm real estate (land+buildings) only; excl. implements AOD002 & livestock AOD003 for cross-year comparability
    ...
    base %>% mutate(census_pop=col("AUM001"), enslaved=0,
      ...
      farmv_total=col("AUK001"),  # AUK001 ("Fences and buildings" in NHGIS) is full farm real estate incl. land (national total $13.3bn, between 1880 and 1900 real estate); excl. implements AUK002 & livestock AUK003
    ...
    base %>% mutate(census_pop=col("AYM001"), enslaved=0,
      ...
      farmv_total=col("AWW001")+col("AWW002"),  # land+improvements + buildings; excl. implements AWW003 & livestock AWW004 for cross-year comparability
