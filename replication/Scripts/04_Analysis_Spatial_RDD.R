...
# using the rd2d package (>= 1.0.0) of Cattaneo, Titiunik, and Yu. Covariate
# adjustment uses the package's native covs.eff efficiency-adjustment option
# (common covariate coefficient across treatment sides).
#
# Specifications:
...
#        {DATA_DIR}/Geology_v2/USGS_*/                  (00_Build_Geology_v2.R)
#        {DATA_DIR}/Geology_v2/Soil/                    (00_Build_Geology_v2.R)
#
...
if (utils::packageVersion("rd2d") < "1.0.0")
  stop("rd2d >= 1.0.0 is required (covs.eff covariate adjustment). ",
       "Run install_dependencies.R from the replication root.")
...
# Boundary-average treatment effect, equal-weighted across the J evaluation
# points, using the bias-corrected (q) point estimates and their full pointwise
# covariance matrix. rd2d (>= 1.0.0) stores the latter in params.cov$main, so
# every call below requests params.cov = "main".
extract_aate <- function(res, n_pts) {
  w <- rep(1/n_pts, n_pts)
  est <- res$main$estimate.q
  cov_mat <- res$params.cov$main
  aate <- sum(w * est)
...
run_rd2d_base <- function(sf_data, outcome_col, border_coords, n_pts, label,
  ...
  t <- d$treatment
  cat(sprintf("  %s (%d obs)...\n", label, length(Y)))
  res <- rd2d(Y = Y, X = X, assignment = t, b = as.matrix(border_coords),
              kernel = kernel, kernel_type = kernel_type, bwselect = bwselect,
              vce = vce, p = p, q = q, params.cov = "main")
...
# Covariate-adjusted boundary RDD via rd2d's native covs.eff option. The outcome
# and covariates are filtered to rows with finite outcome and complete
# covariates; rd2d residualizes the covariates on the local-polynomial basis and
# solves for a common adjustment coefficient across treatment sides (covs.drop
# drops redundant columns if the residualized covariate matrix is rank
# deficient). Returns the same shape as run_rd2d_base, so extract_aate applies.
run_rd2d_cov <- function(sf_data, outcome_col, cov_cols, border_coords, n_pts, label,
                         kernel = "tri", kernel_type = "prod", bwselect = "mserd",
                         p = 1, q = 2, vce = "hc1") {
  d <- filter(sf_data, !is.na(.data[[outcome_col]]) & is.finite(.data[[outcome_col]]))
  Z <- as.matrix(st_drop_geometry(d)[, cov_cols, drop = FALSE])
  ok <- stats::complete.cases(Z)
  d <- d[ok, ]; Z <- Z[ok, , drop = FALSE]
  Y <- d[[outcome_col]]
  X <- st_coordinates(d)
  t <- d$treatment
  cat(sprintf("  %s (%d obs, %d covs)...\n", label, length(Y), ncol(Z)))
  res <- rd2d(Y = Y, X = X, assignment = t, b = as.matrix(border_coords),
              covs.eff = Z, kernel = kernel, kernel_type = kernel_type,
              bwselect = bwselect, vce = vce, p = p, q = q, params.cov = "main")
  aate <- extract_aate(res, n_pts)
  cat(sprintf("    AATE = %.3f (SE = %.3f, p = %.4f); covs used = %d, dropped = %d\n",
              aate$coef, aate$se, aate$p,
              res$opt$N.covs.used, length(res$opt$covs.dropped)))
  list(res = res, aate = aate, label = label, n_obs = length(Y))
}
...
read_geology <- function(type, yr) {
  p <- file.path(DATA_DIR, paste0("Geology_v2/USGS_", type, "/", type, "_", yr, ".csv"))
...
read_soilvar <- function(var, yr) {
  p <- file.path(DATA_DIR, paste0("Geology_v2/Soil/", var, "_", yr, ".csv"))
  if (!file.exists(p)) return(NULL)
  read_csv(p, show_col_types = FALSE) %>%
    mutate(GISJOIN = as.character(GISJOIN)) %>%
    distinct(GISJOIN, .keep_all = TRUE)
...
prepare_county_sf <- function(yr) {
  ...
  centroids <- centroids %>%
    mutate(
      ...
      # Log transform, matching the event study and the Table 2 notes. Counties
      # with zero farm value are dropped; none fall within the border sample.
      log_farmv = ifelse(!is.na(farmv) & farmv > 0, log(farmv), NA)
    )
...
# Ruggedness (TRI) is excluded from the covariate set: TRI magnitudes are
# grid-spacing-dependent (a county TRI is not well defined independent of the
# DEM resolution), and computed on the same 30 m NED grid as slope the two are
# near-duplicates (county-level corr 0.997), which destabilizes the covs.eff
# adjustment. Slope carries the terrain information in interpretable units.
balance_vars <- list(
...
cov_cols_geog  <- c("slope_mean", "elevation_mean",
...
for (yr in seq(1850, 1900, by = 10)) {
  ...
  for (sp in outcome_specs) {
    ...
    rd_geog <- run_rd2d_cov(sf_yr, "log_farmv", cov_cols_geog, border_100$coords_matrix, N_PTS,
                            ...
                            kernel = sp$kernel, kernel_type = sp$kernel_type,
                            bwselect = sp$bwselect, p = sp$p, q = sp$q)
    aate_geog <- extract_aate(rd_geog$res, N_PTS)
    all_outcome_results[[length(all_outcome_results) + 1]] <- data.frame(
    ...
    if (sp$label == "tri_mserd") {
      geog_bw <- as.matrix(rd_geog$res$main[, c("h01", "h02", "h11", "h12")])
      for (bv in balance_vars) {
        ...
        res_bal <- rd2d(Y = d_bal[[bv$col]], X = st_coordinates(d_bal), assignment = d_bal$treatment,
                        b = as.matrix(border_100$coords_matrix),
                        h = geog_bw, kernel = sp$kernel, kernel_type = sp$kernel_type,
                        vce = "hc1", p = sp$p, q = sp$q, params.cov = "main")
    ...
    if (yr <= 1860) {
      rd_slave <- run_rd2d_cov(sf_yr, "log_farmv", cov_cols_slave, border_100$coords_matrix, N_PTS,
                               ...
                               kernel = sp$kernel, kernel_type = sp$kernel_type,
                               bwselect = sp$bwselect, p = sp$p, q = sp$q)
      aate_slave <- extract_aate(rd_slave$res, N_PTS)
      all_outcome_results[[length(all_outcome_results) + 1]] <- data.frame(
